...
type Check struct {
	...
	CHistoryRule *mgo.Collection // history rule hit collection
...
func (c *Check) Rules() {
	...
	if total > 0 {
		c.CHistoryRule.Upsert(bson.M{"ip": c.Info.IP, "system": "linux", "type": c.Info.Type}, bson.M{"$set": bson.M{
...
func (c *Check) Run() {
	...
	//
	//if c.Info.Type == "docker" {
	//	aliveCount, totalCount := 0, len(c.Info.Data)
	//	for _, c.V = range c.Info.Data {
	//		if c.V["status"] == "1" {
	//			aliveCount++
	//		}
	//	}
	//	var s1 = aliveCount * 100 / totalCount
	//
	//}
}
...
func ScanMonitorThread() {
	...
	for i := 0; i < 10; i++ {
		go func() {
			...
			c.CHistoryRule = models.DB.C("history_rule")
