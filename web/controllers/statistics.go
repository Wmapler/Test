...
func (c *StatisticsController) Get() {
	...
	beego.Warn("beginDate:", beginDate)
	beego.Warn("endDate:", endDate)
	switch switchType {
	case "line":
		// notice number datetime base line chart
		beego.Warn("beginDate:", beginDate)
		beego.Warn("endDate:", endDate)
		if len(endDate) > 0 && len(beginDate) > 0 {
			result = getLineDataWithDate(beginDate, endDate)
		}else {
			result = getLineData()
		}
	case "pie":
...
func getLineDataWithDate(beginDate string, endDate string) bson.M {
	beego.Warn("进入到了getLineDataWithDate函数")
	beego.Warn("beginDate:", beginDate)
	beego.Warn("endDate:", endDate)
	var result bson.M
	cli := models.NewNotice()
	...
	beginTime, err1 := time.Parse(settings.TimeFormat, beginDate)
	if err1 != nil {
		beego.Error("beginTime Parse Error!", err1)
	}
	endTime, err2 := time.Parse(settings.TimeFormat, endDate)
	if err2 != nil {
		beego.Error("endTime Parse Error!", err2)
	}
	// get date
	last7date := utils.LastDateStr(settings.TimeFormat, beginTime, endTime)
	//tomorrow := utils.TodayRounded().AddDate(0, 0, 1)
	//sevenDayAgo, _ := time.Parse(settings.TimeFormat, last7date[len(last7date)-1])

	// get count per day for each level in last 7 day
	//last7date = utils.ReverseStrList(last7date)
	beego.Warn("开始查询数据库！")
	matchDate := bson.M{"$gte": beginTime, "$lt": endTime}
	for level := 0; level < 3; level++ {
		//var data []float64
		data := make([]float64, len(last7date))
		matchQuery := bson.M{"time": matchDate, "level": level, "status": settings.ValidNoticeQ["status"]}
		mgoResult := cli.CountPerDay(matchQuery)
		for index, date := range last7date {
			var count float64
