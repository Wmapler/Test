...
func LastDateStr(timeformat string,beginTime time.Time, endTime time.Time) []string {
	beego.Warn("进入到了LastDateStr函数")
	if timeformat == "" {
		timeformat = "01-02"
	}
	var lastDateList []string
	for  {
		lastDateList = append(lastDateList, beginTime.Format(timeformat))
